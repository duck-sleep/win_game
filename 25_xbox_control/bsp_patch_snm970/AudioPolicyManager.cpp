                                                     ...
                                                     bool fromCache)
{
    ...
    // SNM970-HapticX: this board has no physical speaker. When a playback
    // capture (loopback render policy mix) is active, primary module outputs
    // must not stay routed to the speaker backend: its HAL write blocks
    // forever, freezing the mixer thread so the secondary output (tee)
    // patches never receive data and the capture records silence. Force
    // these outputs onto the wired headphone sink while it is connected.
    bool playbackCaptureActive = false;
    for (size_t i = 0; i < mPolicyMixes.size(); i++) {
        if (is_mix_loopback_render(mPolicyMixes.itemAt(i)->mRouteFlags)) {
            playbackCaptureActive = true;
    ...
    if (playbackCaptureActive
            && outputDesc->mPolicyMix.promote() == nullptr
            && mPrimaryOutput != nullptr
            && outputDesc->sharesHwModuleWith(mPrimaryOutput)) {
        sp<DeviceDescriptor> hpDevice =
                mAvailableOutputDevices.getDeviceByType(AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
        if (hpDevice == nullptr) {
            hpDevice = mAvailableOutputDevices.getDeviceByType(AUDIO_DEVICE_OUT_WIRED_HEADSET);
        }
        if (hpDevice != nullptr) {
            ALOGD("%s: playback capture active, output %d rerouted from %s onto %s", __func__,
                    ...
                    hpDevice->toString().c_str());
            return DeviceVector(hpDevice);
        }
    }
